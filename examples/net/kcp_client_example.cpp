#include "dbase/log/log.h"
#include "dbase/net/ikcp.h"
#include "dbase/net/inet_address.h"
#include "dbase/net/socket.h"
#include "dbase/net/socket_ops.h"
#include "dbase/time/time.h"

#include <array>
#include <chrono>
#include <cstdint>
#include <cstring>
#include <string>
#include <thread>
#include <vector>

#if defined(_WIN32)
#include <WinSock2.h>
#include <WS2tcpip.h>
#else
#include <cerrno>
#include <sys/socket.h>
#include <sys/types.h>
#endif

namespace
{
constexpr std::uint32_t kConv = 10086;
constexpr std::uint32_t kToken = 0x12345678;
constexpr std::uint16_t kServerPort = 9000;
constexpr const char* kServerIp = "127.0.0.1";
constexpr int kRecvBufferSize = 2048;
constexpr int kKcpRecvBufferSize = 4096;

[[nodiscard]] bool isWouldBlock()
{
#if defined(_WIN32)
    const int err = ::WSAGetLastError();
    return err == WSAEWOULDBLOCK;
#else
    return errno == EAGAIN || errno == EWOULDBLOCK;
#endif
}

struct ClientContext
{
        dbase::net::Socket* socket{nullptr};
        dbase::net::InetAddress serverAddr;
};

int udpOutput(const char* buf, int len, ikcpcb*, void* user)
{
    auto* ctx = static_cast<ClientContext*>(user);
    if (ctx == nullptr || ctx->socket == nullptr || !ctx->socket->valid())
    {
        return -1;
    }

    const int sent = static_cast<int>(
            ::sendto(
                    ctx->socket->fd(),
                    buf,
                    len,
                    0,
                    ctx->serverAddr.getSockAddr(),
                    ctx->serverAddr.length()));

    if (sent < 0)
    {
        DBASE_LOG_ERROR("client sendto failed");
        return -1;
    }

    return sent;
}
}  // namespace

int main()
{
    dbase::log::setDefaultPatternStyle(dbase::log::PatternStyle::SourceFunction);
    dbase::log::setDefaultLevel(dbase::log::Level::Info);

    auto initRet = dbase::net::SocketOps::initialize();
    if (!initRet)
    {
        DBASE_LOG_FATAL("SocketOps initialize failed: {}", initRet.error().message());
        return 1;
    }

    auto socket = dbase::net::Socket::createUdp(AF_INET);
    if (!socket.valid())
    {
        DBASE_LOG_FATAL("create udp socket failed");
        dbase::net::SocketOps::cleanup();
        return 1;
    }

    socket.setNonBlock(true);

    dbase::net::InetAddress localAddr(0, false, false);
    socket.bindAddress(localAddr);

    ClientContext context;
    context.socket = &socket;
    context.serverAddr = dbase::net::InetAddress(std::string(kServerIp), kServerPort);

    ikcpcb* kcp = ikcp_create(kConv, &context);
    if (kcp == nullptr)
    {
        DBASE_LOG_FATAL("ikcp_create failed");
        dbase::net::SocketOps::cleanup();
        return 1;
    }

    ikcp_settoken(kcp, kToken);
    ikcp_setoutput(kcp, udpOutput);
    ikcp_wndsize(kcp, 128, 128);
    ikcp_nodelay(kcp, 1, 20, 2, 1);

    const std::vector<std::string> messages = {
            "hello",
            "this is client message 1",
            "this is client message 2",
            "dbase kcp demo",
            "bye"};

    for (const auto& message : messages)
    {
        const int ret = ikcp_send(kcp, message.data(), static_cast<int>(message.size()));
        if (ret < 0)
        {
            DBASE_LOG_FATAL("client ikcp_send failed, ret={}", ret);
            ikcp_release(kcp);
            dbase::net::SocketOps::cleanup();
            return 1;
        }

        DBASE_LOG_INFO("client send: {}", message);
    }

    int recvCount = 0;
    std::uint32_t nextUpdate = static_cast<std::uint32_t>(dbase::time::steadyNowMs());

    while (recvCount < static_cast<int>(messages.size()))
    {
        std::array<char, kRecvBufferSize> udpBuffer{};

        for (;;)
        {
            sockaddr_storage fromStorage{};
            socklen_t fromLen = static_cast<socklen_t>(sizeof(fromStorage));

            const int n = static_cast<int>(
                    ::recvfrom(
                            socket.fd(),
                            udpBuffer.data(),
                            static_cast<int>(udpBuffer.size()),
                            0,
                            reinterpret_cast<sockaddr*>(&fromStorage),
                            &fromLen));

            if (n < 0)
            {
                if (isWouldBlock())
                {
                    break;
                }

                DBASE_LOG_ERROR("client recvfrom failed");
                ikcp_release(kcp);
                dbase::net::SocketOps::cleanup();
                return 1;
            }

            const int ret = ikcp_input(kcp, udpBuffer.data(), n);
            if (ret < 0)
            {
                DBASE_LOG_WARN("client ikcp_input failed, ret={}", ret);
            }
        }

        const auto now = static_cast<std::uint32_t>(dbase::time::steadyNowMs());
        if (static_cast<std::int32_t>(now - nextUpdate) >= 0)
        {
            ikcp_update(kcp, now);
            nextUpdate = ikcp_check(kcp, now);
        }

        for (;;)
        {
            std::array<char, kKcpRecvBufferSize> buffer{};
            const int n = ikcp_recv(kcp, buffer.data(), static_cast<int>(buffer.size()));
            if (n < 0)
            {
                break;
            }

            std::string message(buffer.data(), static_cast<std::size_t>(n));
            DBASE_LOG_INFO("client recv: {}", message);
            ++recvCount;
        }

        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }

    for (int i = 0; i < 200; ++i)
    {
        const auto now = static_cast<std::uint32_t>(dbase::time::steadyNowMs());
        ikcp_update(kcp, now);

        std::array<char, kRecvBufferSize> udpBuffer{};
        for (;;)
        {
            sockaddr_storage fromStorage{};
            socklen_t fromLen = static_cast<socklen_t>(sizeof(fromStorage));
            const int n = static_cast<int>(
                    ::recvfrom(
                            socket.fd(),
                            udpBuffer.data(),
                            static_cast<int>(udpBuffer.size()),
                            0,
                            reinterpret_cast<sockaddr*>(&fromStorage),
                            &fromLen));

            if (n < 0)
            {
                if (isWouldBlock())
                {
                    break;
                }
                break;
            }

            (void)ikcp_input(kcp, udpBuffer.data(), n);
        }

        std::this_thread::sleep_for(std::chrono::milliseconds(5));
    }

    DBASE_LOG_INFO(
            "client stats: rtt={}ms pktloss={}%% tx_bw={}B/s rx_bw={}B/s",
            ikcp_rtt(kcp),
            ikcp_pktloss(kcp),
            ikcp_tx_bandwidth(kcp),
            ikcp_rx_bandwidth(kcp));

    ikcp_release(kcp);
    dbase::net::SocketOps::cleanup();
    return 0;
}